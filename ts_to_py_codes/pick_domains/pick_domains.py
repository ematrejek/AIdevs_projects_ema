...
prompt = '''From now on, focus on generating concise, keyword-based queries optimized for web search.
...
async def pick_domains(vars, provider):
    """
    Funkcja generująca zapytania wyszukiwania na podstawie zapytania użytkownika.
    
    Args:
        vars (dict): Słownik zawierający zapytanie użytkownika pod kluczem 'query'
        provider: Obiekt dostawcy do generowania odpowiedzi
        
    Returns:
        dict: Słownik zawierający zapytania wyszukiwania
    """
    # Formatowanie dostępnych domen
    domains = "\n".join([f"{resource['name']}: {resource['url']}" for resource in resources])
    
    # Przygotowanie promptu z wstawionymi domenami
    formatted_prompt = prompt.replace("{domains}", domains)
    
    # Generowanie odpowiedzi za pomocą providera
    messages = [
        {
            "role": "system",
